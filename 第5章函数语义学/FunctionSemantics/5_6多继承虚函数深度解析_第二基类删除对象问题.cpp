...
public:
	...
	//~Derive(){};//即使子类析构函数不写virtual，且与父类的虚析构函数不同名，但是仍会认为子类的析构函数也是虚析构函数
	virtual ~Derive() 
	//会在这安插代码，来调用Base Base2的析构函数。
	{
		int abc;
...
int main()
{
	...
	double d = 0;
	double *pd = &d;
	double *pd1 = (double *)(pd + 1);//这里加1相当于加一个double类型，即+8字节
	double *pd2 = (double *)((char *)pd + 1);
	 ...
	 *		这个析构函数就会被系统调用,但是delete的仍旧是pb2开头这段内存（只删了Base2这部分内存），所以一定报异常。因为这段内存压根就不是new起始的内存
	 *		(会把系统搞混乱。)；
	...
	d.4)Derive里就就算没有虚析构函数，因为Base2里 有虚析构函数，编译器也会为此给Derive生成虚析构函数，为了调用~Base2()和~Base()虚析构函数；
		s即使Derive里有虚析构函数，编译器也会为其穿插代码，使其调用~Base2()\ .~Base()的虚析构函数.
	...
	//当遇到 Base2 *pb2 = new Derive()；delete pb2；会去调整this指针，把this指针从Base2的开始位置调节到Derive的开始位置。(为下一步调用Derive析构函数做准备。)
	(2)调用Derive析构函数
	 */
	p11212->g();
	p11212->i(); //走虚函数表
