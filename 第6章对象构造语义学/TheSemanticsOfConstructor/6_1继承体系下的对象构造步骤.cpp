 ...
 * 三：构造函数中对虚函数的调用
 *	 构造函数中调用虚函数，不是走虚函数表调用，因此也不是多态。(理论上也是能调用虚函数的，因为此时虚函数表指针已经被初始化过了，可能是编译器为了简便统一处理不。)
 *	 但是在构造函数中调用虚函数，这个虚函数再去调用其他虚函数，可能走虚函数表，实现多态，不同编译器实现不一样
...
public:
	...
	{
		//memcpy(this,)//千万不要这样做，会覆盖虚函数表指针。
		myvirfunc(); //构造函数中，这里没有走虚函数表，而是直接通过虚函数地址，直接调用这个虚函数（静态方式调用）(直接调用)
		printf("C this = %p\n", this);
		cout << "C::C()" << endl;
	}
	virtual ~C() {}
	virtual void myvirfunc() { myvirfunc2(); }//调用myvirfunc2()是通过虚函数表调用的(间接的通过构造函数来间接调用也能走虚函数调用。)
	virtual void myvirfunc2() {}
	int m_c;
