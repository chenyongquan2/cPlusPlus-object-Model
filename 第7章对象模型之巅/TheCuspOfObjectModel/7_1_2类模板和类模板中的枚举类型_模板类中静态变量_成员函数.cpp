...
public:
	...
	//普通成员函数
	void myFunction01() const{ cout << "myFunction01被调用" << endl; }//在不掉用的情况下不会被实例化。
...
void func()
{
	...
	myEnum02 = MyClass<double>::ECUS_Free;//这两行在编译器眼中仅仅当作一个类型。
	//结论：类中的枚举类型依赖于具体的类模板类型，但是和类模板关系不大(并不会实例化这个类模板)


	//2.2静态成员变量--类本身并没有被实例化，只是成员变量被实例化(当作一个普通变量的感觉)
