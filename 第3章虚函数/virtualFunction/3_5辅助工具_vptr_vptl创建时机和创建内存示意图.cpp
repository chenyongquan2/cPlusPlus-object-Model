...
* cl.exe 编译链接工具
*	1.切换到cpp所在的路径
	//语法：cl(这是L) /d1(这是123的1) reportSingleClassLayoutDerived 3_4多重继承虚函数表分析.cpp
...
*	实际上，对于这种有虚函数的类，在编译的时候，编译器会往相关的构造函数中增加为vptr赋值的代码。
*	这是编译器为在编译期间增加的代码。
...
*	这样程序运行到创建对象时候调用构造函数，会调用类中构造函数中的给vptr赋值的代码，进行把虚函数表地址赋值给虚函数表指针，这样虚函数表指针就有值了。 
