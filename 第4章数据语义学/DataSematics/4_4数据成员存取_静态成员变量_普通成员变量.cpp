...
int main()
{
	...
	//静态成员变量的地址相同(编译器就已经确定了)，是一个对象，地址相同
	...
	pmyobj->myfunc();
	//编译器角度--插入一个隐藏的参数this指针：MYACLS::myfunc(pmyobj)来调用
	//上面函数的声明MYACLS::myfunc(MYACLS *const this)
	//{
	//this->m_i = 5;
	//this->m_j = 5;
	//}
 ...
 *访问时使用类对象地址加成员变量偏移值进行访问。（查看汇编，可发觉编译时就已经确定好了偏移值了 ）
 *注意：
