...
int main()
{
	...
	printf("myobj.m_i = %p\n", &myobj.m_i); //对象的成员变量是有真正的内存地址的。因此可通过指针操作内存。
	...
	*p2 = 30;
	...
	int MYACLS::*mypoint(得加上类名，表示是类内部的) = &MYACLS::m_j;
	...
	int *ptest = 0;
	int MYACLS::*mypoint2;
	mypoint2 = 0; //成员变量指针赋值为0，编译器给你减1(防止你成员变量为0，表示指向成员变量的开头位置)(-1表示无效的偏移量)
