...
CTemValue DoubleMyself(CTemValue&t)
{
	...
	//return  tem;//生成一个临时对象作为返回值，然后调用拷贝构造函数把tem对象拷贝到主调函数的内存空间。

	//优化后
	return CTemValue(t.value1 * 2, t.value2 * 2);//生成一个临时对象直接返回,对比上面少了一次拷贝构造
}
//编译器视角
void DoubleMyself(CTemValue&obj,CTemValue&t)//编译器会插入第一个参数。
{
