...
//		 * 生成一个临时对象，
//			X obj_tmp;//声明
//			obj_tmp.X::X(1000);//调用类型转换构造函数，
//			obj_x.X::operator=(obj_tmp);//调用拷贝赋值运算符进行赋值给obj_x;
//		 * 	obj_tmp.X::~X()然后调用析构函数释放临时对象obj_tmp。
//		 */
//		m_a = 500;
//	}
//};
...
public:
	X obj_x;
	int m_a;
	A(int temvalue):obj_x(1000)//大家把初始化列表中的代码看成是函数体代码的一部分。
	 /*
	站在编译器的角度
	X obj_x;
	obj_x.X::X(1000);
